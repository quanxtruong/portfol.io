const fs = require("fs");
const { parseCoursework, parseCSV } = require("./parser");
...
class GPA {
  ...
  async init(filePath) {
    try {
      ...
      this.csvAbsolutePath =
        "/Users/quantruong/portfol.io/backend/data/" + courseworkCsvPath;
  ...
  getPointsAndHours() {
    const pastCourses = this.courseworkData.filter(
      (course) =>
        course.Type === "In-Residence" && course.Grade && course.Grade !== "CR"
    );
  ...
  calculateCumulativeGPA() {
    ...
    return (
      Math.floor((this.cumulativePoints / this.cumulativeHours) * 10000) / 10000
    );
  }
  ...
  getAllClasses() {
    return this.courseworkData
      .filter(
        (course) =>
          (course.Type === "In-Residence" || course.Type === "Transfer") &&
          course.Grade != "CR"
      )
      .map((course) => ({
  ...
  addCourse(course, isMajor) {
    ...
    const csvFilePath =
      "/Users/quantruong/portfol.io/backend/data/ut_courses.csv";
    const jsonFilePath =
      "/Users/quantruong/portfol.io/backend/data/coursework.json"; // Path to save data
    ...
    return new Promise((resolve, reject) => {
      fs.createReadStream(csvFilePath)
        ...
        .on("data", (row) => {
          if (
            row["Course ID"] === upperCourse ||
            row["Course Name"].toUpperCase() === upperCourse
          ) {
            record = row;
          }
        })
        .on("end", () => {
          ...
          if (
            this.courseworkData.some(
              (item) => item["Course ID"] === record["Course ID"]
            )
          ) {
          ...
          fs.writeFile(
            jsonFilePath,
            JSON.stringify(this.courseworkData, null, 2),
            (err) => {
              if (err) {
  ...
  removeCourse(courseId) {
    ...
    this.courseworkData = this.courseworkData.filter(
  ...
  updateGrade(courseId, grade) {
    ...
    if (course.Type != "In-Residence" || course.Semester == "Course Bank") {
      course.Grade = grade;
      return true;
    }
    ...
    if (grade === null || grade === "") {
      ...
      if (course.Grade) {
        // Only subtract hours if a grade exists
      ...
      if (course.Grade === "" || course.Grade === null) {
        ...
        this.cumulativePoints +=
          (GRADE_POINTS[grade] - oldGradePoints) * creditHours;
        if (course["Major Course"] === "Yes") {
          this.majorPoints +=
            (GRADE_POINTS[grade] - oldGradePoints) * creditHours;
        }
  ...
  updateSemester(courseId, newSemester) {
    ...
    const wasInCourseBank = oldSemester === "Course Bank";
    const isMovingToCourseBank = newSemester === "Course Bank";
    ...
    if (isMovingToCourseBank && !wasInCourseBank) {
      // Revert GPA changes when moving back to "Course Bank"
      if (course.Grade) {
      ...
      console.log(
        `Reverted GPA changes for course ID ${courseId} moved to Course Bank.`
      );
    } else if (!isMovingToCourseBank && wasInCourseBank) {
      // Add GPA contribution when moving out of "Course Bank"
      if (course.Grade) {
        this.cumulativePoints += gradePoints * creditHours;
        this.cumulativeHours += creditHours;
      ...
      console.log(
        `Updated GPA for course ID ${courseId} moved out of Course Bank.`
      );
    }

    // Save updated coursework data
    const jsonFilePath =
      "/Users/quantruong/portfol.io/backend/data/coursework.json";
    fs.writeFileSync(
      jsonFilePath,
      JSON.stringify(this.courseworkData, null, 2)
    );

    console.log(`Updated semester for course ID ${courseId} to ${newSemester}`);
    return true;
  }
  ...
  toggleMajor(courseId) {
    ...
    if (course["Major Course"] === "Yes") {
      ...
      if (course.Grade && course.Semeseter != "Course Bank") {
        this.majorPoints -= gradePoints * creditHours;
        this.majorHours -= creditHours;
      }
